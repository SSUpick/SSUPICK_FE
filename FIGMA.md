...
### 세로 viewport 대응 — `short:` / `tall:` 변형

캔버스 스타일(인트로/스플래시) 페이지는 viewport 높이가 디자인 기준(844px)에서 벗어나면 요소가 겹치거나 너무 커보일 수 있다. [src/index.css](./src/index.css)에 정의된 변형으로 임계점에서 살짝 조정한다.
...
**언제 쓰나**: 캔버스/풀-블리드 페이지에서 §7의 viewport 검증 시 충돌/과대표시가 보일 때. 일반 스크롤 페이지에는 불필요.
